...
async function processCandidate(candidate) {
  ...
  const candidateDetailUrl = window.location.href;
...
function navigateToCandidateDetailPagination(candidate) {
  if (candidate.elementRef) {
    const topElement = candidate.elementRef.querySelector('.top-I0hDlGQcuR');
    if (topElement) {
      topElement.click();
      return true;
    }
  ...
  const nameElements = document.querySelectorAll('.name-zRouov6HCt span');
  
  for (const nameElement of nameElements) {
    if (nameElement.textContent.trim() === candidate.name) {
      const candidateContainer = nameElement.closest('.container-i9WQ6tolSM');
      if (candidateContainer) {
        const topElement = candidateContainer.querySelector('.top-I0hDlGQcuR');
        if (topElement) {
          topElement.click();
          return true;
        }
  ...
  for (const tr of items) {
    ...
    if (nameElement) {
      ...
      if (nameMatch && nameMatch[0] === candidate.name) {
        const itemElement = tr.querySelector('.item-NpfQ8Ve_W4');
        if (itemElement) {
          itemElement.click();
          return true;
        }
